...
function SummariserContent() {
  ...
  const [summarizeMode, setSummarizeMode] = useState<'transcript' | 'audio'>('transcript');
  ...
  const generateToolContent = useCallback(async (mode: string) => {
    ...
    setToolLoading(mode);
    setError(null);
    const endpoint = summarizeMode === 'audio' ? '/api/audio-summarise' : '/api/summarize';
    try {
  ...
  const ReportControlBar = ({
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-b border-blue-100/50 dark:border-white/5 bg-white/60 dark:bg-slate-800/40 backdrop-blur-xl gap-4 relative z-10">
      <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
        ...
        <GlassDropdown
          options={[
            { id: 'detailed_summary', label: 'Detailed Summary', icon: Sparkles },
            { id: 'points_oriented', label: 'Points Oriented', icon: ListOrdered },
            { id: 'timestamped_highlights', label: 'Highlights', icon: Clock },
            { id: 'reading_mode', label: 'Reading Mode', icon: BookOpen },
          ]}
  ...
  return (
    <DashboardContainer>
      <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="bg-white/60 dark:bg-slate-900/50 backdrop-blur-xl border border-blue-100/50 dark:border-white/10 rounded-3xl p-5 md:p-6 shadow-sm hover:shadow-md transition-shadow max-w-4xl mx-auto w-full relative z-20">
          <div className="flex flex-col md:flex-row md:items-center gap-5">
            ...
            <form onSubmit={handleGenerate} className="flex-1 flex flex-col sm:flex-row gap-3 w-full relative">
              ...
              >
                ...
                {summarizeMode === 'audio' ? 'Audio Summarise' : 'Summarise'}
              </button>
            </form>
          </div>
          ...
          <div className="flex items-center gap-4 mt-4 pt-4 border-t border-slate-100 dark:border-white/5">
            <div className="flex items-center gap-1 p-1 bg-slate-100/50 dark:bg-white/5 rounded-xl border border-slate-200/50 dark:border-white/5">
              <button
                onClick={() => setSummarizeMode('transcript')}
                className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${summarizeMode === 'transcript' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm border border-slate-200/50 dark:border-white/10' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
              >
                Transcript Mode
              </button>
              <button
                onClick={() => setSummarizeMode('audio')}
                className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${summarizeMode === 'audio' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm border border-slate-200/50 dark:border-white/10' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
              >
                Audio Mode
              </button>
            </div>
            {summarizeMode === 'audio' && (
              <span className="text-[9px] font-bold text-blue-500 uppercase tracking-widest animate-pulse">
                Using audio-based AI (slower but works without captions)
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-10 animate-in fade-in slide-in-from-top-4 duration-1000 w-full max-w-5xl mx-auto">
            ...
            <div className="w-full space-y-6">
              ...
              {activeTool === 'podcast_script' && toolResults.podcast_script && (
                <div id="tool-podcast_script" className="bg-white/60 dark:bg-slate-900/60 flex flex-col rounded-3xl border border-blue-100/50 dark:border-white/10 shadow-sm backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-700 overflow-hidden">
                  <ReportControlBar
                    language={selectedLanguage}
                    setLanguage={setSelectedLanguage}
                    currentTool={activeTool}
                  ...
                  <div className="p-8 md:p-12">
                    <div className="flex items-center gap-4 mb-8">
                      <div className="p-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-2xl shadow-sm border border-rose-100/50">
                        <MessageSquare size={20} />
                      </div>
                      <div>
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight">Podcast Script</h2>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Conversational format</p>
                      </div>
                    </div>
                    
                    <div className="prose prose-slate dark:prose-invert max-w-none prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-p:leading-relaxed prose-p:text-sm md:prose-p:text-base prose-p:mb-6 prose-strong:text-blue-600 dark:prose-strong:text-blue-400 bg-slate-50/30 dark:bg-white/[0.02] p-8 rounded-2xl border border-blue-100/30 dark:border-white/5 shadow-inner">
                      <ReactMarkdown>
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-100 dark:border-white/5 flex justify-start">
                      <TTSButton text={getStringContent(toolResults.podcast_script)} language={selectedLanguage} />
                    </div>
                  </div>
                </div>
              )}
